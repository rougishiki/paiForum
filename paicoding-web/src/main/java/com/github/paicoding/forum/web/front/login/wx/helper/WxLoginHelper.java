...
public class WxLoginHelper {
    ...
    public SseEmitter subscribe() throws IOException {
        String deviceId = ReqInfoContext.getReqInfo().getDeviceId();//ThreadLocal里获得设备id
        String realCode = deviceCodeCache.getUnchecked(deviceId);//之前没有则会调用load方法生成一个5分钟有效期的映射
                                                                // 有则会返回已有的对应的验证码
        // fixme 设置15min的超时时间, 超时时间一旦设置不能修改；因此导致刷新验证码并不会增加连接的有效期
        SseEmitter sseEmitter = new SseEmitter(SSE_EXPIRE_TIME);//new一个15分钟的SSE连接
        SseEmitter oldSse = verifyCodeCache.getIfPresent(realCode);//如果已存在则关闭之前的
        if (oldSse != null) {
            oldSse.complete();
        }
